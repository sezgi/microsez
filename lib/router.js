...
Router.map(function() {
  ...
  // server routes
  this.route('randomPost', {
    where: 'server',
    path: '/randomPost',
    action: function () {
      var randomNumber = _.random(Comments.find({ votes: { $gte: 0 } }).count() - 1),
          randomComment = Comments.find({ votes: { $gte: 0 } }, { limit: 1, skip: randomNumber }).fetch()[0],
          randomPost = Posts.find(randomComment.postId).fetch();
      randomPost[0].comments = [ randomComment ];
      randomPost[0].truncated = true;
      this.response.writeHead(200, {'Content-Type': 'application/json'});
      this.response.end(JSON.stringify(randomPost));
    }
  })
  this.route('search', {
    where: 'server',
